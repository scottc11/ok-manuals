...
import Code from "../../components/manual/Code";
import DownloadList from "../../components/manual/DownloadList";
import { TbCircuitPushbutton } from "react-icons/tb";

const IMG = "/images/manuals/counterpoint";

const Label = ({ children }: { children: React.ReactNode }) => (
  <span className="text-azure font-unica p-1 rounded-sm">{children}</span>
);
const Highlight = ({ children }: { children: React.ReactNode }) => (
  <span className="text-black bg-lime font-mono px-1 rounded-sm">{children}</span>
);
const Micro = ({ children }: { children: React.ReactNode }) => (
  <span className="text-onyx/60 italic text-sm">{children}</span>
);

function RateToCvTable() {
  const rows = [
    ["0V - 1.25V", "1/4"],
    ["1.25V - 2.5V", "1/4t"],
    ["2.5V - 3.75V", "1/8"],
  ...
  return (
    <table className="lg:w-1/2 w-full mb-4 border-collapse">
      ...
      <tbody>
        {rows.map(([cv, rate]) => (
          <tr key={cv} className="border-b border-dotted border-black pt-4">
            <td>{cv}</td>
            <td>{rate}</td>
          </tr>
        ))}
...
export default function CounterpointManualPage() {
  return (
      ...
      <ContentsContextProvider>
        <Section>
          ...
          <div className="flex flex-row my-4 justify-center">
            <ManualImage source={`${IMG}/panel.png`} alt="Counterpoint panel" />
          </div>
        </Section>
        ...
        <Section>
          ...
          <div className="bg-white p-4 rounded-lg font-serif my-4 shadow-onyx/30 shadow-md">
            ...
            <p><span className="text-onyx/60 italic mr-2">noun</span></p>
            ...
            <p><span className="text-onyx/60 italic mr-2">verb</span></p>
            <p className="ml-4">
              <span className="text-onyx/70 mr-2">1.</span>
              <span className="text-onyx/60 italic mr-2">Music</span>
          ...
          <p>This is <Label>COUNTERPOINT</Label>. It is a Eurorack module designed to <u>control multiple oscillators</u> in a modular system, with the core focus being to keep each oscillator (or &quot;voice&quot;) musically coherent with one another.</p>
          ...
          <p>The module is very hands on and <b>performative</b>. It features a sequencer that allows the performer to capture harmonic progressions and ideas on the fly and play them back as sequenced loops which can be further manipulated in real time through a variety of gestures.</p>
          ...
          <p>For more info on the name, check out <ManualLink external href="https://en.wikipedia.org/wiki/Counterpoint">this wikipedia page</ManualLink>.</p>
        </Section>
        ...
        <Section>
          ...
          <div className="flex flex-row m-6 justify-center">
            ...
            <img className="bg-panel p-4 w-3/4" src={`${IMG}/panel_jacks.svg`} alt="panel jacks" />
          </div>
          <p>The input / output jacks are split into two sections. On the right side there is a grid of jacks corresponding to each channel (channel <Label>A</Label>, <Label>B</Label>, <Label>C</Label>, <Label>D</Label>), and on the left side are jacks corrosponding to the clock transport and arpeggiator.</p>
          ...
          <table className="w-full my-4 border-collapse">
            <thead><tr className="border-b-2 border-black"><th className="text-left w-32">Label</th><th className="text-left">Description</th></tr></thead>
            <tbody>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>1VO</Label></td><td>1 volt per octave output (-1V to 9V range)</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>GATE</Label></td><td>5V gate signal</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>B CV</Label></td><td>CV control over channel slew OR &quot;pitch bend&quot; of 1VO output</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>Q CV</Label></td><td>When a channel is in Quantizer mode, the voltage present on this input will be quantized to the nearest scale degree</td></tr>
            </tbody>
          </table>
          ...
          <table className="w-full my-4 border-collapse">
            <thead><tr className="border-b-2 border-black"><th className="text-left w-32">Label</th><th className="text-left">Description</th></tr></thead>
            <tbody>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>CLOCK</Label></td><td>A clock signal must be present here in order for time related functions to work. See the <b>Clocking</b> section for more information.</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>RESET</Label></td><td>Reset any active sequences to beat one. (it will wait till the next quarter note occurs before resetting)</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>DIR CV</Label></td><td>CV control over the direction of the arpeggiator.</td></tr>
              <tr className="border-b-2 border-onyx/10 border-dotted"><td className="font-bold pr-4"><Label>RATE CV</Label></td><td>CV control over the rate of the arpeggiator.</td></tr>
            </tbody>
          </table>
        </Section>
        ...
        <Section>
          ...
          <Note><p>Note: The internal clock will never exceed 240 BPM nor go below 40 BPM (regardless of the clock signal present at the CLOCK input jack).</p></Note>
        </Section>
        ...
        <Section>
          ...
          <div className="flex flex-col md:flex-row items-center">
            ...
            <div className="basis-full md:basis-1/3">
              <div className="flex flex-row justify-center h-48 m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel shadow-onyx/50 shadow-lg p-4" src={`${IMG}/channel_mode_symbols.svg`} alt="monophonic mode symbols" />
              </div>
            </div>
          </div>
          ...
          <div className="flex flex-row justify-center m-4 gap-8">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img className="w-1/3 bg-panel shadow-onyx/50 shadow-lg p-4" src={`${IMG}/bend_mode_toggle.svg`} alt="bend mode toggle" />
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img className="w-1/3 bg-panel shadow-onyx/50 shadow-lg p-4" src={`${IMG}/quant_toggle.svg`} alt="quantizer mode symbols" />
          </div>
          ...
          <div className="flex flex-row justify-center m-4">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img className="w-1/3 bg-panel shadow-onyx/50 shadow-lg p-4" src={`${IMG}/arp_toggle.svg`} alt="arpeggiator mode symbols" />
          </div>
        ...
        <Section>
          ...
          <div className="flex flex-col md:flex-row">
            <div className="basis-full md:basis-3/4">
              <p>The 8 toggle switches on the <Label>Counterpoint</Label> are what you use to contruct a common musical scale between all 4 channels. They logic behind them is centered around music theory, particularly the concept of &quot;Counterpoint and Harmony&quot;. You don&apos;t need to know any music theory to use the module, but if you&apos;re interested in the theory behind it you can check out <ManualLink external href="https://en.wikipedia.org/wiki/Counterpoint">this wikipedia page</ManualLink>.</p>
              ...
              <table className="w-full mb-8 border-collapse text-sm">
                ...
                <tbody>
                  <tr><td>8 (top)</td><td>DOWN</td><td>Tonic octave</td></tr>
                  <tr><td>7</td><td>CENTER</td><td>Major 7th</td></tr>
                  <tr><td>6</td><td>CENTER</td><td>Major 6th</td></tr>
                  <tr><td>5</td><td>CENTER</td><td>Perfect 5th</td></tr>
                  <tr><td>4</td><td>CENTER</td><td>Perfect 4th</td></tr>
                  <tr><td>3</td><td>UP</td><td>Major 3rd</td></tr>
                  <tr><td>2</td><td>UP</td><td>Major 2nd</td></tr>
                  <tr><td>1 (bottom)</td><td>UP</td><td>Tonic</td></tr>
                </tbody>
              </table>
              ...
              <table className="w-full mb-8 border-collapse text-sm">
                ...
                <tbody>
                  <tr><td>8 (top)</td><td>DOWN</td><td>Tonic octave</td></tr>
                  <tr><td>7</td><td>DOWN</td><td>Minor 7th</td></tr>
                  <tr><td>6</td><td>DOWN</td><td>Minor 7th</td></tr>
                  <tr><td>5</td><td>CENTER</td><td>Perfect 5th</td></tr>
                  <tr><td>4</td><td>CENTER</td><td>Perfect 4th</td></tr>
            ...
            <div className="order-first sm:order-last basis-full md:basis-1/4">
              <div className="flex flex-row justify-center h-96 m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel p-4" src={`${IMG}/scale_degree_switches.svg`} alt="scale degree switches" />
              </div>
            </div>
          </div>
        </Section>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="basis-full order-first md:order-last md:basis-1/4">
              <div className="flex flex-row justify-center h-48 md:h-96 m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel p-4" src={`${IMG}/bend_controls.svg`} alt="bend controls" />
              </div>
            </div>
            <div className="basis-full md:basis-3/4">
              ...
              <table className="w-full mb-8 border-collapse">
                <thead><tr className="border-b-2 border-black"><th className="text-left w-32">Mode</th><th className="text-left">Description</th></tr></thead>
                <tbody>
                  <tr><td className="font-bold pr-4"><Label>PORTAMENTO</Label></td><td>Smoothly move between scale degrees</td></tr>
                  <tr><td className="font-bold pr-4"><Label>PITCH BEND</Label></td><td>Converts the control voltage present on the <Label>B CV</Label> input to an unquantized value between 1 and 12 semitones (in both directions)</td></tr>
                </tbody>
              </table>
              ...
              <div className="flex flex-row justify-center m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="w-1/3 bg-panel p-2" src={`${IMG}/bend_mode_toggle.svg`} alt="bend mode toggle" />
              </div>
              ...
              <Note><p>Note: the CV input range for the <Label>B CV</Label> input is <b>-10V to 10V</b>.</p></Note>
            </div>
          </div>
        </Section>
        ...
        <Section>
          ...
          <div className="flex flex-col md:flex-row gap-8">
            <div className="basis-full order-first md:order-last md:basis-1/4">
              <div className="flex flex-row justify-center m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel h-48 md:h-96 p-4" src={`${IMG}/panel_arp.svg`} alt="Arpeggiator" />
              </div>
            </div>
            <div className="basis-full md:basis-3/4">
          ...
          <Note><p>Note: Enabling / disabling the arpeggiator will only work if a channel is in <Label>MONOPHONIC</Label> mode. If it is in <Label>QUANTIZER</Label> mode, the arpeggiator will not be enabled / disabled.</p></Note>
        ...
        <Section>
          ...
          <div className="flex flex-row m-6 justify-center">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img className="bg-panel p-4 w-2/3" src={`${IMG}/panel_seq_actions.svg`} alt="sequencer actions" />
          </div>
          ...
          <div className="flex flex-row m-4 justify-center">
            {/* eslint-disable-next-line @next/next/no-img-element */}
            <img className="bg-panel py-4 px-8 w-2/3" src={`${IMG}/sequence_length_meter.svg`} alt="sequence length and meter" />
          </div>
          ...
          <div className="flex flex-row items-center">
            <div className="basis-full order-first md:order-last md:basis-1/3">
              <div className="flex flex-row justify-center m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel p-4" src={`${IMG}/seq_length.svg`} alt="sequence length" />
              </div>
            </div>
            <div className="basis-full md:basis-2/3">
          ...
          <Note><p>How <Label>AUTO</Label> sequence length works: As long as <Label>RECORD</Label> is enabled and bars is set to <Label>AUTO</Label>, the length of a sequence will automatically increase in increments of one bar as time passes. Once you feel the sequence is long enough, you can press the <Label>RECORD</Label> button again to disable recording.</p></Note>
          <SectionSubheading title="Sequence meter" />
          <div className="flex flex-row items-center">
            <div className="basis-full order-first md:order-last md:basis-1/3">
              <div className="flex flex-row justify-center m-4">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="bg-panel p-4" src={`${IMG}/seq_meter.svg`} alt="sequence meter" />
              </div>
            </div>
            <div className="basis-full md:basis-2/3">
          ...
          <Note><p><b>Sequence Override:</b> If a sequence is playing back and <Label>RECORD</Label> <b>is not</b> enabled, interacting with the touch pads will override the sequence and output + hold the last touched scale degree. Releasing your finger will re-activate playback of the sequence.</p></Note>
        ...
        <Section>
          ...
          <table className="w-full mb-4 border-collapse">
            <thead><tr className="border-b-2 border-black"><th className="text-left w-1/3">Gesture</th><th className="text-left w-2/3">Effect</th></tr></thead>
            <tbody>
              <tr className="border-b border-dotted border-black pt-4"><td><Label>FUNC</Label> + <Label>SELECT PAD</Label> + <Label>RECORD</Label></td><td>Adds one bar to the actively running sequence, or creates a new sequence with a length of one bar.</td></tr>
              <tr className="border-b border-dotted border-black pt-4"><td><Label>{`FUNC`}</Label> + <Label>OCTAVE PAD</Label></td><td>If a sequence is playing back, Applies an <b>octave offset</b> to the active sequence of the corrosponding channel. (top octave pad +1 octave, bottom octave pad -1 octave).</td></tr>
              <tr className="border-b border-dotted border-black pt-4"><td><Label>{`FUNC`}</Label> + <Label>OCTAVE PAD</Label></td><td>If no sequence is playing back, sets the octave of a channel without triggering a new note. Only when a degree pad is touched will the target octave be reflected on the channels output.</td></tr>
              <tr className="border-b border-dotted border-black pt-4"><td><Label>{`FUNC`}</Label> + <Label>ARP RATE PAD</Label></td><td>Sets the arp rate to a triplet of the corrosponding note value.</td></tr>
            </tbody>
          </table>
        </Section>
        ...
        <Section>
          ...
          <p>If the LED is illuminated, then the Q CV input will set the rate at which the <Label>GATE</Label> output will be triggered. The rate at which the <Label>GATE</Label> output will be triggered is determined like so:</p>
          <RateToCvTable />
          ...
          <p>The row labeled <Label>AQ</Label> is used to indicate whether the channel should automatically quantize a recorded sequence to a 16th note grid. If the LED is illuminated, then the module will automatically quantize sequences when <Label>RECORD</Label> is enabled.</p>
          <SectionSubheading title="Gate Sync (GS)" />
          <p>The row labeled <Label>GS</Label> is used to synchronize the gate output of one or more channels. If any of the LEDs in the row are illuminated, the channels associated with those LEDs will have their <Label>{`GATE`}</Label> output synchronized with other &quot;gate synced&quot; channels. This means that when one channel&apos;s <Label>{`GATE`}</Label> output goes HIGH, all gate synced channels will also have their <Label>{`GATE`}</Label> output also go HIGH.</p>
        </Section>
        ...
        <Section>
          ...
          <p>Saved sequences can be accessed by pressing the <Label>SETTINGS</Label> button and entering the settings menu. The top for rows represent the 16 possible sequence slots. If a slot is illuminated, then a sequence is saved in that slot.</p>
          <p>To launch a sequence, you need only touch the sequence slot you wish to launch. Once launched, the sequence will be loaded into either the selected channels (via channel <Label>SELECT PAD</Label>s), or if no channel is selected it will get loaded into the corrosponding channel which the sequence slot is associated with. After a sequence is loaded, it becomes &quot;queued&quot; for playback, and will not begin playing back until the start of the next bar.</p>
          <p>To delete a sequence, touch the sequence you wish to delete. The CLEAR button will start pulsing, at this point pressing the CLEAR button will delete the selected sequence.</p>
          <p>You can copy a sequence from one slot to another by touching the sequence you wish to copy and then touching the sequence slot you wish to copy it to. The CLEAR button will start pulsing, at this point pressing the CLEAR button will copy the selected sequence to the selected sequence slot.</p>
        </Section>
          ...
          <div className="flex flex-row">
            <div className="flex flex-col basis-3/4">
              <div className="flex flex-row justify-center py-12">
                {/* eslint-disable-next-line @next/next/no-img-element */}
                <img className="w-1/4" src="/images/manuals/midi_logo.svg" alt="MIDI Logo" />
              </div>
              <p>Using a <ManualLink external href="https://minimidi.world/">MIDI-to-TRS (TYPE-A)</ManualLink> adapter, Counterpoint can both send and recieve MIDI messages via the 2HP <Label>MIDI EXPANDER</Label> (sold separately).</p>
              <p>The MIDI implementation is standard, but since Counterpoint&apos;s interface of touch pads don&apos;t suggest any particular &quot;notes&quot;, using it as a traditional MIDI controller may not give you the results you expect.</p>
              <p>The <Label>MIDI EXPANDER</Label> has a switch on it which is used to determine the direction of the MIDI messages. This means Counterpoint cannot both send and recieve MIDI messages at the same time.</p>
              ...
              <table className="w-full my-4 border-collapse">
                <thead><tr className="border-b-2 border-black"><th className="text-left w-1/3">Counterpoint Channel</th><th className="text-left w-2/3">MIDI Channel</th></tr></thead>
                <tbody>
                  <tr className="border-b border-dotted border-black pt-4"><td>A</td><td>Channel 1</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>B</td><td>Channel 2</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>C</td><td>Channel 3</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>D</td><td>Channel 4</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>A, B, D, C (all channels)</td><td>Channel 5</td></tr>
                </tbody>
              </table>
              ...
              <table className="w-full my-4 border-collapse">
                <thead><tr className="border-b-2 border-black"><th className="text-left w-1/3">Channel</th><th className="text-left w-1/3">Event</th><th className="text-left w-2/3">Handling</th></tr></thead>
                <tbody>
                  <tr className="border-b border-dotted border-black pt-4"><td>Channel (1, 2, 3, 4)</td><td>Note ON</td><td>Sets <Label>1VO</Label> to the corrosponding note + sets the <Label>GATE</Label> output to HIGH.</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>Channel (1, 2, 3, 4)</td><td>Note OFF</td><td>Sets the gate output to LOW.</td></tr>
                  <tr className="border-b border-dotted border-black pt-4"><td>Channel 5</td><td>Note ON</td><td>Cyclic: cycles through all channels in a cyclic manner.</td></tr>
                </tbody>
              </table>
            </div>
            <div className="flex flex-row justify-center basis-1/4">
              {/* eslint-disable-next-line @next/next/no-img-element */}
              <img className="h-1/2" src="/images/manuals/midi_expander.png" alt="MIDI EXPANDER" />
            </div>
          </div>
        </Section>
        ...
        <Section>
          ...
          <p>You will need to align the frequency of all your oscillators prior to calibration so that they are all nearly the same frequency. <b>This is important for the calibration process</b>. If one oscillators frequency prior to calibration is significantly higher or lower then the others then the initial pitch after calibration will be a semi-tones higher/lower then the other channels.</p>
        </Section>

        <Section>
          <SectionHeading title="📝 Specifications" />
          <table className="w-full mb-4 border-collapse">
            <thead><tr className="border-b-2 border-black"><th className="text-left">Parameter</th><th className="text-left">Value</th></tr></thead>
            <tbody>
              <tr className="border-b border-dotted border-black pt-4"><td className="pt-2">Power consumption</td><td className="pt-2">+12V rail 60mA, -12V rail 10mA, 5V rail n/a</td></tr>
              <tr className="border-b border-dotted border-black"><td className="pt-2">Dimensions</td><td className="pt-2"><p>3U x 42 HP</p><p>10.16cm x 106.68cm</p></td></tr>
              <tr className="border-b border-dotted border-black"><td className="pt-2">Depth</td><td className="pt-2">22mm</td></tr>
              <tr className="border-b border-dotted border-black"><td className="pt-2">Weight</td><td className="pt-2">100g</td></tr>
            </tbody>
          </table>
        </Section>
        ...
        <Section>
          ...
          <Note><p>Note: You will NOT need to power the module from your eurorack system for this process. The USB connection is all that is needed.</p></Note>
